// ──────────────────────────────────────────────────────────────────────────────
//  NAIRU model with three wage-growth series (ULC, AENA & WPI).
//  • Inflation equation uses ULC (series 1) as the wage-cost input.
//  • Wage block estimates separate observation equations for all three wages.
//  • Missing final-quarter observations for any wage are inferred jointly.
// ──────────────────────────────────────────────────────────────────────────────

data {
  int<lower=1> T;
  int<lower=1> J;
  matrix[T, J] Y;                      // cols: ULC, AENA, WPI, import, LUR, CPI, PIE, d1, d2, d3, d4
  int<lower=0, upper=1> wage_obs[T, 3];
  int<lower=0, upper=T> missing_wage_index[3];
}
...
transformed data {
  ...
  for (t in 1:T) {
    import_demeaned[t] = Y[t, 4] - import_mean;
    if (wage_obs[t, 1] == 1) {
    ...
    if (wage_obs[t, 3] == 1) {
      wage3_demeaned[t] = Y[t, 3] - wage_sum[3];
    } else {
      wage3_demeaned[t] = 0;
    }
  }
...
parameters {
  ...
  vector<lower = -5,   upper = 7>[5]     wage3_init;
  real<lower = 0,   upper = 0.40>        tau;
  vector<lower = -2.3, upper = 3.7>[3]   wage_missing;
  ...
  real<lower = -0.30, upper = 1.90>      delta_wage3_0;
  real<lower = -5,   upper = 1>          gamma_wage3_0;
  vector<lower = -4,   upper = 2>[2]     gamma_wage3_lag;
  real<lower = -6,   upper = 0>          lambda_wage3_0;
  vector<lower = -9,   upper = 9>[2]     xi_wage3;
...
model {
  ...
  for (k in 1:2) {
    ...
    gamma_wage3_lag[k] ~ normal(pow(0.5, k) * -2, 1);
  }
...

  {
    ...
    for (t in 8:T) {
      ...
      real ugap_now = gamma_pt_0 * ((Y[t, 5] - NAIRU[t]) / Y[t, 5]);
      real mom_now  = lambda_pt_0 * (Y[t-1, 5] - Y[t-2, 5]) / Y[t, 5];
      real imp_now  = alpha_pt_0 * (import_demeaned[t-1] - import_demeaned[t-2]);
      real wage_now = phi_pt_0 * wage1_demeaned[t-1];

      for (k in 1:4) {
        ugap_now += gamma_pt_lag[k] * ((Y[t-k, 5] - NAIRU[t-k]) / Y[t-k, 5]);
      ...
      pt_hat[t] = exp_now + ugap_now + mom_now + imp_now + wage_now
                + xi_pt[1] * Y[t, 8] + xi_pt[2] * Y[t, 9];
    }

    for (t in 6:T) {
      real exp1  = delta_wage1_0 * Y[t, 7];
      real ugap1 = gamma_wage1_0 * (1 - NAIRU[t] / Y[t, 5]);
      real mom1  = lambda_wage1_0 * (Y[t-1, 5] - Y[t-2, 5]) / Y[t, 5];

      real exp2  = delta_wage2_0 * Y[t, 7];
      real ugap2 = gamma_wage2_0 * (1 - NAIRU[t] / Y[t, 5]);
      real mom2  = lambda_wage2_0 * (Y[t-1, 5] - Y[t-2, 5]) / Y[t, 5];

      real exp3  = delta_wage3_0 * Y[t, 7];
      real ugap3 = gamma_wage3_0 * (1 - NAIRU[t] / Y[t, 5]);
      real mom3  = lambda_wage3_0 * (Y[t-1, 5] - Y[t-2, 5]) / Y[t, 5];

      for (k in 1:2) {
        ugap1 += gamma_wage1_lag[k] * (1 - NAIRU[t-k] / Y[t-k, 5]);
        ugap2 += gamma_wage2_lag[k] * (1 - NAIRU[t-k] / Y[t-k, 5]);
        ugap3 += gamma_wage3_lag[k] * (1 - NAIRU[t-k] / Y[t-k, 5]);
      }

      wage1_hat[t] = exp1 + ugap1 + mom1
                   + xi_wage1[1] * Y[t, 10] + xi_wage1[2] * Y[t, 11];

      wage2_hat[t] = exp2 + ugap2 + mom2
                   + xi_wage2[1] * Y[t, 10] + xi_wage2[2] * Y[t, 11];

      wage3_hat[t] = exp3 + ugap3 + mom3
    ...
    for (t in 1:T) {
      ...
      if (wage_obs[t, 3] == 1) {
        target += normal_lpdf(Y[t, 3] | wage3_hat[t], eps_wage3);
      }
    }
    ...
    if (missing_wage_index[3] > 0) {
      target += normal_lpdf(wage_missing[3] | wage3_hat[missing_wage_index[3]], eps_wage3);
    }
  }
}
