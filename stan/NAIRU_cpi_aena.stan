...
//  (π_t ≡ Y[,4]  and  aena_t ≡ Y[,1]).
//
//  • Keeps the original DATA and TRANSFORMED DATA blocks unchanged.
//  • Replaces lagged-π and lagged-aena terms with lags (up to three / two)
//    of the *other* regressors (expectations, unemployment gap, momentum,
//    import-price shocks, ΔAENA_demeaned), while dummies remain contemporaneous.
...
data {
  ...
  int<lower=0, upper=1> aena_obs[T];
  int<lower=0, upper=T> missing_aena_index;
}

transformed data {
  vector[T] Y2_demeaned;        // demeaned ΔP_IMP  (col 2)
  vector[T] Y1_demeaned;        // demeaned ΔAENA    (col 1)
...
parameters {
  ...
  vector<lower = -5,   upper = 7>[5]     puhat_init;
  real<lower = 0,   upper = 0.40>        tau;
  real<lower = -2.3, upper = 3.7>        aena_missing;
  ...
  vector<lower = -2.00, upper = 1.00>[4] lambda_pt_lag;
...
model {
  ...
  for (k in 1:4) {
    ...
    phi_pt_lag[k]    ~ normal(pow(0.5,k) * 0.06 , 0.50);   // ΔAENA_demeaned
...
//    lambda_pu_lag[k] ~ normal(pow(0.5,k) * -3   , 1);
  ...
  delta_pu_0  ~ normal(delta_pu_prior_mean , 0.10);
  ...
  // ── State & Observation Equations ─────────────────────────────────────────
  {
    ...
    for (t in 8:T) {
      ...
      real aena_now  = phi_pt_0 * Y1_demeaned[t-1];
...
//                    (Y[t-1-k,3] - Y[t-2-k,3]) / Y[t-k,3];
        ...
        aena_now  += phi_pt_lag[k] *
                    Y1_demeaned[t-1-k];
      }

      pt_hat[t] = exp_now + ugap_now + mom_now + imp_now + aena_now
...
//                    (Y[t-1-k,3] - Y[t-2-k,3]) / Y[t-k,3];
    ...
    if (missing_aena_index > 0) {
      for (t in 1:T) {
        if (t != missing_aena_index && aena_obs[t] == 1) {
          target += normal_lpdf(Y[t,1] | pu_hat[t], eps_pu);
        }
      }
      target += normal_lpdf(aena_missing | pu_hat[missing_aena_index], eps_pu);
    } else {
...
//                  (Y[t-1-k,3] - Y[t-2-k,3]) / Y[t-k,3];
      imp_now  += alpha_pt_lag[k]  *
                  (Y2_demeaned[t-1-k] - Y2_demeaned[t-2-k]);
      aena_now  += phi_pt_lag[k]    * Y1_demeaned[t-1-k];
    }

    pt_residuals[t] = Y[t,4] - (exp_now + ugap_now + mom_now + imp_now
                               + aena_now + xi_pt[1]*Y[t,6] + xi_pt[2]*Y[t,7]);
  }
}
