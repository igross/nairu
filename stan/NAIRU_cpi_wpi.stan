...
//  (π_t ≡ Y[,4]  and  wpi_t ≡ Y[,1]).
//
//  • Keeps the original DATA and TRANSFORMED DATA blocks unchanged.
//  • Replaces lagged-π and lagged-wpi terms with lags (up to three / two)
//    of the *other* regressors (expectations, unemployment gap, momentum,
//    import-price shocks, ΔWPI_demeaned), while dummies remain contemporaneous.
...
data {
  ...
  int<lower=0, upper=1> wpi_obs[T];
  int<lower=0, upper=T> missing_wpi_index;
}
...
parameters {
  ...
  real<lower = -2.3, upper = 3.7>        wpi_missing;
...
model {
  ...
  for (k in 1:4) {
    ...
    phi_pt_lag[k]    ~ normal(pow(0.5,k) * 0.06 , 0.50);   // ΔWPI_demeaned
...
//    lambda_pu_lag[k] ~ normal(pow(0.5,k) * -3   , 1);
  ...
  // ── State & Observation Equations ─────────────────────────────────────────
  {
    ...
    for (t in 8:T) {
      ...
      real wpi_now  = phi_pt_0 * Y1_demeaned[t-1];
...
//                    (Y[t-1-k,3] - Y[t-2-k,3]) / Y[t-k,3];
        ...
        wpi_now  += phi_pt_lag[k] *
                    Y1_demeaned[t-1-k];
      }

      pt_hat[t] = exp_now + ugap_now + mom_now + imp_now + wpi_now
...
//                    (Y[t-1-k,3] - Y[t-2-k,3]) / Y[t-k,3];
    ...
    if (missing_wpi_index > 0) {
      for (t in 1:T) {
        if (t != missing_wpi_index && wpi_obs[t] == 1) {
          target += normal_lpdf(Y[t,1] | pu_hat[t], eps_pu);
        }
      }
      target += normal_lpdf(wpi_missing | pu_hat[missing_wpi_index], eps_pu);
    } else {
...
//                  (Y[t-1-k,3] - Y[t-2-k,3]) / Y[t-k,3];
      imp_now  += alpha_pt_lag[k]  *
                  (Y2_demeaned[t-1-k] - Y2_demeaned[t-2-k]);
      wpi_now  += phi_pt_lag[k]    * Y1_demeaned[t-1-k];
    }

    pt_residuals[t] = Y[t,4] - (exp_now + ugap_now + mom_now + imp_now
                               + wpi_now + xi_pt[1]*Y[t,6] + xi_pt[2]*Y[t,7]);
  }
}
